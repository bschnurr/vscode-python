...
suite('Activation - ActivationService', () => {
    [true, false].forEach(jediIsEnabled => {
        suite(`Test activation - ${jediIsEnabled ? 'Jedi is enabled' : 'Jedi is disabled'}`, () => {
            ...
            function createLanguageServerCache() {
                return new LanguageServerCache(
                    workspaceService.object,
                    stateFactory.object,
                    experiments.object,
                    output.object,
                    appShell.object,
                    lsNotSupportedDiagnosticService.object,
                    [],
            ...
            test('LS is supported', async () => {
                ...
                const activationService = createLanguageServerCache();
